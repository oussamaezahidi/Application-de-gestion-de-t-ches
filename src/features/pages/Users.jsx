...
function Users() {
  ...
  useEffect(() => {
    Promise.all([fetchUsersAPI(), fetchTasksAPI()])
  ...
  const handleDeleteUser = async (userId) => {
    ...
    if (userTasks.length > 0) {
      ...
      try {
        ...
        setTasks(
          tasks.map((t) =>
            String(t.userId) === String(userId) ? { ...t, userId: "" } : t,
          ),
        );
        ...
        {users.length === 0 ? (
          ...
          <div className="users-list list-group">
            {users.map((user) => {
              ...
              const avatarSrc =
                user.avatar ||
                `https://ui-avatars.com/api/?name=${encodeURIComponent(user.username)}&background=6c9eff&color=fff&size=64`;
              return (
                <div
                  key={user.id}
                  className="user-row list-group-item d-flex align-items-center justify-content-between"
                >
